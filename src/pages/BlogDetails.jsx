...
import blogData from "../mocks/articlesData.json";
...
const BlogDetails = () => {
  ...
  const images = import.meta.glob("../assets/img/blog/*.webp", {
    eager: true /* for image */,
  });

  useEffect(() => {
    // Find the current blog using the `blogTitle` from URL params
    const currentBlog = blogData.articlesData.find(
      (blog) => blog.title.replace(/\s+/g, "-") === blogTitle
    );
    ...
    if (currentBlog) {
      ...
      const blogWithImage = {
        ...currentBlog,
        img: imageModule.default || imageModule, // Use the `default` export or the module itself
      };
      setBlog(blogWithImage);
    } else {
      setBlog(null); // Blog not found
    }
  }, [blogTitle]);
  ...
  return (
    <div className="blog-details">
      {blog?.img && (
        <img className="hero-bg" src={blog?.img} alt={blog.title} />
      )}
